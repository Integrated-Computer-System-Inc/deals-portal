...
import { ShieldAlert, ArrowRight, Loader2, Fingerprint, Lock } from 'lucide-react';
...
function LoginForm() {
  ...
  const handleDemoLogin = async () => {
    setIsLoading(true);
    await signIn('demo-credentials', { accountName: 'Demo User', callbackUrl: '/dashboard' });
  };
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="w-full space-y-3">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex items-center justify-center w-full px-6 py-2.5 text-xs font-medium text-zinc-400 hover:text-white transition-all bg-transparent rounded-xl hover:bg-white/5 border border-transparent hover:border-white/10"
          >
            <span>Continue as Demo User</span>
            <ArrowRight className="w-3.5 h-3.5 ml-2 text-zinc-500" />
          </button>
        </div>
